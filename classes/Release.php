...
class Release extends Conexao{

	public function getReleases($limit = ''){
		...
		if ($limit != ''){
			$sql = "SELECT *, DATE_FORMAT(cli_data, '%d.%m.%Y') AS notdata 
								FROM cms_release 
								WHERE rel_data <= '".date('Y-m-d')."' 
								ORDER BY rel_data DESC
								LIMIT $limit";
		}else{
			$sql = "SELECT *, DATE_FORMAT(rel_data, '%d.%m.%Y') AS notdata 
								FROM cms_release 
								WHERE rel_data <= '".date('Y-m-d')."' 
								ORDER BY rel_data DESC";
		}
	...
	public function getRelease($id){
		...
		$stmt = $pdo->prepare("SELECT *, DATE_FORMAT(rel_data, '%d.%m.%Y') AS notdata 
								FROM cms_release 
								WHERE rel_id = ?");
